...
class InputMontoDeuda extends StatefulWidget {
  const InputMontoDeuda({
    super.key,
  ...
  final MontoDeudaServicio monto;
  final double? montoDeuda;
  final void Function(MontoDeudaServicio montoDeudaServicio) onChangeMonto;

  @override
  State<InputMontoDeuda> createState() => _InputMontoDeudaState();
}

class _InputMontoDeudaState extends State<InputMontoDeuda> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.montoDeuda != null &&
        widget.monto.value.isNotEmpty &&
        double.parse(widget.monto.value) < widget.montoDeuda!) {
      helperMessage = 'Monto mínimo permitido: ${CtUtils.formatCurrency(
        widget.monto.montoMinimoDeduda,
        ...
        children: [
          ...
          Expanded(
            child: Container(
              ...
              child: TextFormField(
                enabled: widget.monto.montoMaximoDeuda != widget.monto.montoMinimoDeduda,
                onTapOutside: (event) {
