...
class SelectDestinoCredito extends StatelessWidget {
  const SelectDestinoCredito({
    super.key,
    required this.value,
    required this.onChanged,
    required this.destinosCredito,
  });
  final DestinoCredito? value;
  final List<DestinoCredito> destinosCredito;
  final void Function(DestinoCredito destino) onChanged;
            ...
            children: [
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  final destinoCredito = destinosCredito[index];
                  return ListTile(
                    ...
                    selected: destinoCredito.codigoDestino == value?.codigoDestino,
                    selectedColor: AppColors.primary700,
                    title: Text(destinoCredito.descripcionDestino),
                    onTap: () {
                      onChanged(destinoCredito);
                      Navigator.pop(context);
                    },
                  );
                },
                itemCount: destinosCredito.length,
              )
            ],
          ),
        );
