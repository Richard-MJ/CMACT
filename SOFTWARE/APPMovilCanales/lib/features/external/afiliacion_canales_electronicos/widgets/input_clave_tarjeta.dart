...
class InputClaveTajeta extends ConsumerStatefulWidget {
  const InputClaveTajeta({
    super.key,
    required this.value,
    required this.onChanged,
  });

  final ClaveTarjeta value;
  final void Function(ClaveTarjeta value) onChanged;

  @override
  InputClaveTarjetaState createState() => InputClaveTarjetaState();
}

class InputClaveTarjetaState extends ConsumerState<InputClaveTajeta> {
  @override
  void initState() {
    super.initState();
    setState(() {
    ...
    _focusNode.addListener(() {
      if (!_focusNode.hasFocus) {
        widget.onChanged(ClaveTarjeta.dirty(widget.value.value));
      }
    });
  }
          ...
          Expanded(
            child: TextFormField(
              ...
              onChanged: (value) {
                widget.onChanged(
                  widget.value.isPure
                      ? ClaveTarjeta.pure(value)
                      : ClaveTarjeta.dirty(value),
                );
              },
