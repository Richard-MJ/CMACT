...
public class ClienteConfiguracion : IEntityTypeConfiguration<Cliente>
{
    ...
    private void ConfigurarCliente(EntityTypeBuilder<Cliente> builder)
    {
        ...
        builder
            .HasOne(c => c.PersonaFisica)
            .WithOne(c => c.Cliente)
            .HasForeignKey<PersonaFisica>(p => new { p.CodigoEmpresa, p.CodigoCliente })
            .IsRequired(false);

        builder
            .HasOne(c => c.PersonaJuridica)
            .WithOne(c => c.Cliente)
            .HasForeignKey<PersonaJuridica>(p => new { p.CodigoEmpresa, p.CodigoCliente })
            .IsRequired(false);
    }
}
