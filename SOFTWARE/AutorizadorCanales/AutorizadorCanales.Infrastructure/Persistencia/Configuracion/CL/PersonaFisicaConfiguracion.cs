...
public class PersonaFisicaConfiguracion : IEntityTypeConfiguration<PersonaFisica>
{
    ...
    {
        ConfigurarPersonaFisica(builder);
    }

    private static void ConfigurarPersonaFisica(EntityTypeBuilder<PersonaFisica> builder)
    {
        ...
        builder.HasOne(f => f.Cliente)
            .WithOne(f => f.PersonaFisica)
            .HasForeignKey<PersonaFisica>(f => new { f.CodigoEmpresa, f.CodigoCliente })
            .IsRequired(false);

        builder.HasOne(f => f.Conyugue)
            .WithMany()
            .HasForeignKey(f => new { f.CodigoEmpresa, f.CodigoClienteConyugue })
            .IsRequired(false);
    }
}
